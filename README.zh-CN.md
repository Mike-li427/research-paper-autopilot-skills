...
完整 skill 清单在 `skills_manifest.json`。每个 skill 的作用、仓库路径和 GitHub 直达地址见 `INCLUDED_SKILLS.md`。第三方来源和再分发提醒在 `THIRD_PARTY_SKILLS.md`。

## 第三方 Skill 说明

这个仓库是一个打包整合项目，不代表里面每个 skill 都是本仓库原创。`skills/` 下面很多 supporting skills 来自已有的本地/community/第三方 Codex skill 安装，本仓库把它们 vendored 进来，是为了让别人克隆后可以一键安装完整科研论文工作流。

仓库顶层 README、安装脚本、校验脚本、manifest、plugin metadata，以及本地 `research-paper-autopilot` 的总控编排和加固文件，属于本仓库的整合工作。其他 supporting skills 可能有各自的作者、上游项目、许可证和再分发条款。

公开发布或商业使用前，请阅读 `THIRD_PARTY_SKILLS.md`，保留各 skill 目录里已有的 license/notice 文件，并在知道上游来源时补充署名。
